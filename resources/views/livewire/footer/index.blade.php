<div>
    <footer class="bg-[#C4C4C424]/10">
        <div class="mx-auto w-full  px-6 lg:pt-8">
            <div class="md:flex px-20 gap-20">
                <div class="mb-6 md:mb-0 w-1/5">
                    <a href="/" class="flex items-center mb-2.5">
                        <img src="../assets/images/logo.svg" class="h-[66px] mr-3" alt="FlowBite Logo" />

                    </a>
                    <h1 class="text-[#454545] text-[13px] font-medium mb-5">Ruko Amira Town House No.10, Dusun II
                        Prompong, Kutasari, Kec. Baturaden, Kabupaten Banyumas, Jawa Tengah 53151
                    </h1>
                </div>

                <div class="">
                    <h2 class="mb-6 text-lg font-semibold"
                        style="text-decoration: underline; text-decoration-color: #4E9FCA; text-decoration-thickness: 3px; text-underline-offset: 4px;">
                        Shop</h2>

                    <ul class="text-[#181818] font-poppins ">
                        <li class="mb-4">
                            <h1 class="text-[#181818] font-poppins">Obat Resep</h1>

                        </li>
                        <li class="mb-4">
                            <h1 class="text-[#181818] font-poppins">Obat Bebas</h1>

                        </li>
                        <li class="mb-4">
                            <h1 class="text-[#181818] font-poppins">Kebutuhan COVID-19</h1>

                        </li>
                        <li class="mb-4">
                            <h1 class="text-[#181818] font-poppins">Ibu & Anak</h1>

                        </li>
                        <li class="mb-4">
                            <h1 class="text-[#181818] font-poppins">Vitamin & Suplemen</h1>

                        </li>
                        <li class="mb-4">
                            <h1 class="text-[#181818] font-poppins">Perawatan Tubuh</h1>

                        </li>
                        <li>
                            <h1 class="text-[#181818] font-poppins">Alat Kesehatan</h1>

                        </li>
                    </ul>
                </div>
                <div>
                    <h2 class="mb-6 text-lg font-semibold"
                        style="text-decoration: underline; text-decoration-color: #4E9FCA; text-decoration-thickness: 3px; text-underline-offset: 4px;">
                        About</h2>
                    <ul class="text-[#181818] font-poppins">
                        <li class="mb-4">
                            <a href="/about" class="text-[#181818] font-poppins ">About Us</a>
                        </li>
                        <li class="mb-4">
                            <a href="/privacy-police" class="text-[#181818] font-poppins ">Privacy Policy</a>
                        </li>
                        <li>
                            <a href="/terms-conditions" class="text-[#181818] font-poppins ">Terms & Conditions</a>
                        </li>
                    </ul>
                </div>
                <div>
                    <h2 class="mb-6 text-lg font-semibold"
                        style="text-decoration: underline; text-decoration-color: #4E9FCA; text-decoration-thickness: 3px; text-underline-offset: 4px;">
                        Contact Us</h2>
                    <ul class="text-gray-600 font-medium">
                        <li class="mb-4">
                            <a class="flex gap-3 items-center cursor-pointer">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M4.2565 16.5933C3.13983 14.7093 2.74924 12.4825 3.15804 10.331C3.56684 8.1794 4.74693 6.25104 6.47677 4.90789C8.2066 3.56474 10.3672 2.89919 12.553 3.03617C14.7388 3.17315 16.7994 4.10325 18.348 5.65186C19.8966 7.20047 20.8267 9.26108 20.9637 11.4469C21.1007 13.6326 20.4351 15.7932 19.0919 17.5231C17.7488 19.2529 15.8204 20.433 13.6689 20.8418C11.5173 21.2506 9.29049 20.86 7.4065 19.7433V19.7433L4.294 20.6246C4.16648 20.6619 4.03128 20.6642 3.90256 20.6313C3.77384 20.5983 3.65635 20.5314 3.5624 20.4374C3.46845 20.3435 3.4015 20.226 3.36858 20.0973C3.33565 19.9686 3.33796 19.8334 3.37525 19.7058L4.2565 16.5933Z"
                                        stroke="#101828" stroke-width="1.5" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                    <path
                                        d="M14.2594 17.25C13.2726 17.2525 12.295 17.0599 11.3828 16.6834C10.4706 16.3069 9.64176 15.7539 8.94395 15.0561C8.24615 14.3583 7.69311 13.5295 7.3166 12.6173C6.9401 11.7051 6.74755 10.7275 6.75002 9.74065C6.7525 9.04608 7.03016 8.3808 7.52217 7.89055C8.01419 7.40029 8.68045 7.12502 9.37502 7.12502V7.12502C9.48962 7.12411 9.60234 7.1541 9.70133 7.21184C9.80032 7.26958 9.88191 7.35294 9.93752 7.45315L11.0344 9.36565C11.0996 9.48191 11.1331 9.61329 11.1315 9.74659C11.1298 9.87989 11.0931 10.0104 11.025 10.125L10.1438 11.5969C10.5952 12.601 11.399 13.4049 12.4031 13.8563L13.875 12.975C13.9896 12.9069 14.1202 12.8702 14.2535 12.8686C14.3868 12.8669 14.5181 12.9004 14.6344 12.9656L16.5469 14.0625C16.6471 14.1181 16.7305 14.1997 16.7882 14.2987C16.8459 14.3977 16.8759 14.5104 16.875 14.625C16.8726 15.3188 16.5965 15.9837 16.1068 16.4752C15.6171 16.9666 14.9532 17.2451 14.2594 17.25V17.25Z"
                                        stroke="#101828" stroke-width="1.5" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                </svg>
                                <div class="">
                                    <h1 class="text-[#454545] text-base font-normal">Whats App</h1>
                                    <h1 class="text-[#454545] text-base font-normal">081215869243</h1>
                                </div>
                            </a>
                        </li>
                        <li class="mb-4">
                            <a class="flex gap-3 items-center cursor-pointer">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M8.67187 11.6998C9.44364 13.2935 10.7324 14.579 12.3281 15.3467C12.4458 15.4024 12.576 15.4265 12.7059 15.4167C12.8358 15.4068 12.9608 15.3633 13.0687 15.2904L15.4125 13.7248C15.516 13.6546 15.6357 13.6117 15.7603 13.6002C15.8849 13.5887 16.0104 13.609 16.125 13.6592L20.5125 15.5435C20.6625 15.6059 20.7877 15.7159 20.869 15.8565C20.9504 15.9971 20.9832 16.1606 20.9625 16.3217C20.8234 17.407 20.2937 18.4046 19.4723 19.1276C18.6509 19.8506 17.5943 20.2495 16.5 20.2498C13.1185 20.2498 9.87548 18.9065 7.48439 16.5154C5.0933 14.1243 3.75 10.8813 3.75 7.49979C3.75025 6.40553 4.1492 5.34886 4.87221 4.5275C5.59522 3.70613 6.59274 3.17635 7.67812 3.03729C7.83922 3.01659 8.00266 3.04943 8.14326 3.13074C8.28386 3.21206 8.39384 3.33733 8.45625 3.48729L10.3406 7.88416C10.3896 7.99699 10.4101 8.12013 10.4003 8.24275C10.3905 8.36537 10.3507 8.48369 10.2844 8.58729L8.71875 10.9685C8.64905 11.0762 8.60814 11.2 8.59993 11.328C8.59172 11.4561 8.61649 11.5841 8.67187 11.6998V11.6998Z"
                                        stroke="#101828" stroke-width="1.5" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                </svg>
                                <div class="">
                                    <h1 class="text-[#454545] text-base font-normal">Phone</h1>
                                    <h1 class="text-[#454545] text-base font-normal">081215869243</h1>
                                </div>
                            </a>
                        </li>
                    </ul>
                </div>
                <div>
                    <h2 class="mb-6 text-lg font-semibold"
                        style="text-decoration: underline; text-decoration-color: #4E9FCA; text-decoration-thickness: 3px; text-underline-offset: 4px;">
                        Follow Us</h2>
                    <ul class="text-gray-600 font-medium">
                        <li class="mb-4">
                            <a class="flex gap-3 items-center cursor-pointer">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M12 15.75C14.0711 15.75 15.75 14.0711 15.75 12C15.75 9.92893 14.0711 8.25 12 8.25C9.92893 8.25 8.25 9.92893 8.25 12C8.25 14.0711 9.92893 15.75 12 15.75Z"
                                        stroke="#101828" stroke-width="1.5" stroke-miterlimit="10" />
                                    <path
                                        d="M16.125 3.375H7.875C5.38972 3.375 3.375 5.38972 3.375 7.875V16.125C3.375 18.6103 5.38972 20.625 7.875 20.625H16.125C18.6103 20.625 20.625 18.6103 20.625 16.125V7.875C20.625 5.38972 18.6103 3.375 16.125 3.375Z"
                                        stroke="#101828" stroke-width="1.5" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                    <path
                                        d="M16.875 8.25C17.4963 8.25 18 7.74632 18 7.125C18 6.50368 17.4963 6 16.875 6C16.2537 6 15.75 6.50368 15.75 7.125C15.75 7.74632 16.2537 8.25 16.875 8.25Z"
                                        fill="#101828" />
                                </svg>
                                <h1 class="text-[#454545] text-base font-normal">Instagram</h1>
                            </a>
                        </li>
                        <li class="mb-4">
                            <a class="flex gap-3 items-center cursor-pointer">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M15.75 9.9375C17.28 11.0361 19.1164 11.6263 21 11.625V7.875C19.6076 7.875 18.2723 7.32188 17.2877 6.33731C16.3031 5.35274 15.75 4.01739 15.75 2.625H12V14.625C12 15.0948 11.8739 15.5559 11.6349 15.9603C11.3959 16.3647 11.0528 16.6976 10.6413 16.9242C10.2298 17.1508 9.76502 17.2629 9.29548 17.2486C8.82594 17.2344 8.36882 17.0944 7.97181 16.8433C7.5748 16.5922 7.25245 16.2392 7.03838 15.821C6.82432 15.4029 6.72639 14.9349 6.75481 14.466C6.78323 13.9971 6.93695 13.5445 7.19995 13.1552C7.46294 12.766 7.82557 12.4544 8.25 12.2531V8.35312C7.05617 8.56716 5.94816 9.11732 5.05605 9.93901C4.16394 10.7607 3.52474 11.8198 3.21349 12.9921C2.90224 14.1643 2.93185 15.4011 3.29885 16.5571C3.66585 17.7131 4.355 18.7404 5.28541 19.5185C6.21583 20.2965 7.34889 20.793 8.5516 20.9497C9.7543 21.1063 10.9767 20.9166 12.0754 20.4029C13.1741 19.8891 14.1034 19.0726 14.7543 18.0492C15.4052 17.0257 15.7506 15.8379 15.75 14.625V9.9375Z"
                                        stroke="#101828" stroke-width="1.5" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                </svg>
                                <h1 class="text-[#454545] text-base font-normal">Tik Tok</h1>
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
            <div class="px-20 mt-10">
                <div class=" border-t border-[#A9ABAE] flex justify-center py-4">
                    <h1 class="text-[#A9ABAE] text-base font-normal">© Copyright Apotek Tugu Batu 2023. All right
                        reserved</h1>
                </div>
            </div>
        </div>
    </footer>

</div>
